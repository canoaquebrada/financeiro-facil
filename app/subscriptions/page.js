    ...
    <>
      {loading ? (
        ...
        <>
          ...
          {status?.plan && status?.status === 'active' && !blocked && (
            <div className="mb-6 rounded-2xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-white p-4 dark:border-emerald-800 dark:from-emerald-950/20 dark:to-slate-900">
              <div className="flex items-center gap-3">
                <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-100 text-lg dark:bg-emerald-800/50">
                  {status.plan === 'lifetime' ? '💎' : status.plan === 'trial' ? '🆓' : '⭐'}
                </span>
                <div>
                  <p className="text-sm font-bold text-emerald-800 dark:text-emerald-200">
                    Plano {planLabel(status.plan)} ativo
                  </p>
                  <p className="text-xs text-emerald-600 dark:text-emerald-400">
                    Aproveite todos os recursos do Financeiro Fácil!
                  </p>
                </div>
              </div>
            </div>
          ...
          {/* Subscription status cards - ALWAYS visible */}
          <div className="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Plano atual</p>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              ...
              <p className={`mt-1 text-xl font-bold capitalize ${
                status?.status === 'active' ? 'text-emerald-600' : status?.status === 'paused' ? 'text-amber-600' : 'text-rose-600'
              }`}>
                {status?.status === 'active' ? 'Ativo' : status?.status === 'paused' ? 'Pausado' : 'Expirado'}
              </p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Tempo restante</p>
              <p className={`mt-1 text-xl font-bold ${daysLeft === null ? 'text-slate-900' : daysLeft <= 3 ? 'text-rose-600' : 'text-emerald-600'}`}>
                {daysLeft === null ? 'Ilimitado' : status?.timeRemaining || `${daysLeft} dias`}
              </p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Expira em</p>
              <p className="mt-1 text-xl font-bold text-slate-900 dark:text-white">
                {status?.subscriptionEnd
                  ? new Date(status.subscriptionEnd).toLocaleDateString('pt-BR')
                  : status?.plan === 'lifetime' ? 'Nunca expira' : '-'}
              </p>
            </div>
          </div>
            ...
            {plans.map((plan) => (
              ...
              >
                ...
                <button
                  ...
                  disabled={purchasing || status?.plan === plan.id}
                ...
                >
                  {purchasing ? 'Processando...' : status?.plan === plan.id ? 'Plano atual' : 'Assinar agora'}
                </button>
              </div>
            ))}
          </div>
